...
The main idea is to have a type system that implies an order among the types. The ordering will then allow us to infer the type of any expression.
...
* $C_i \ T_{i,1} \dots T_{i,k_i}$ a _sort_ of $T$,
...
$$\mathit{Int} ::= \mu C.0 \ | \ \mathit{Pos} \ \mathit{Nat} \ | \ \mathit{Neg} \ \mathit{Nat}$$
...
f(T_1,\dots,T_n) :=& [ \forall a_1 \dots a_n. T]_{\{(a_1,T_1),\dots,(a_n,T_n)\}} \text{ for a given mono type } T.
\end{aligned}
$$
...
\begin{aligned}
  ...
  \lor& \ \Gamma = \Delta \cup \{(C,f)\} \text{ where } T = C \ T_1 \dots T_n \text{ is a type application and } T_i \text{ is a}\\
      ...
      &\text{ is a mono type and } \Delta \text{ is a context}
\end{aligned}
$$
...
$$
\end{addmargin}
We define a order on $$\mathrm{values}(\mathit{Nat})=\lim_{n\to\infty}\{1,\underbrace{\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \dots \ \mathit{Succ} \ 1}_{n \text{ times}}\}=\{1,\mathit{Succ} \ 1,\mathit{Succ} \ \mathit{Succ} \ 1,\dots\}$$
