...
\begin{itemize}
    ...
    \item \textbf{Case} $\mathit{iet} = a$ for $a\in\mathcal{A}$: Then $\semantic{$a$} = a$ and therefore the conclusion holds.
\end{itemize}
```
...
\begin{itemize}
    ...
    \item \textbf{Case} $\mathit{lt} = a \ \mf{":"} \ \mf{"\{v:Int|"}\ \mathit{qt} \ \mf{"\}"} \ \mf{"->"} \ \mathit{lt}_2$ for $a\in\mathcal{V}, \mathit{qt}\in\mf{<qualifier-type>}$ and $\mathit{lt}\in\mf{<liquid-type>}$: From the premise of the inference rule, we assume that $\mf{"\{v:Int|"}\ \mathit{qt} \ \mf{"\}"}:_\Theta\hat{T}_1$ and $\mathit{lt}_2:_{\Theta\cup\{(a,\hat{T}_1)\}}\hat{T}_2$ for liquid types $\hat{T}_1,\hat{T}_2$. By induction hypothesis $\semantic{$\mathit{lt}_2$} = \hat{T}_2$. Then $\semantic{$\mathit{lt}$} = a:\hat{T}_1\to\hat{T}_2$ and therefore the conclusion holds.
\end{itemize}
```
...
s = \lambda n.\lambda m.\begin{cases} \Big\lfloor \frac{n}{m} \Big\rfloor& \text{if } m \neq 0\\
    ...
    \item \textbf{Case} $e = \mf{"\textbackslash"} \ a \ \mf{"->"} \ e$ for $a\in\mathcal{V},e\in\mf{<exp>}$: Then $T = b:\hat{T}_1 \to \hat{T}_2$ for liquid types $\hat{T}_1,\hat{T}_2$ and $b\in\mathcal{V}$. By the premise of the inference rule we assume  $\Gamma,\Delta\cup\{(a,\hat{T}_1)\},\Theta\cup\{(a,\hat{T}_1)\},\Lambda\vdash e: \hat{T}_2$ can be derived. We now need to show that \(\semantic{$e$}_{\Gamma,\Delta'}\in\text{value}_\Gamma(\hat{T}_1\to \hat{T}_2)\). We know \(\semantic{$e$}_{\Gamma,\Delta'}=\lambda b. \semantic{$e$}_{\Gamma,\Delta\cup\{(a,b)\}}\) for $b\in\mathcal{V}$. We will therefore by the definition of the abstraction in the lambda expression let $b\in\text{value}_\Gamma(\overline{\Gamma}(\hat{T}_1))$ and show \(\semantic{$e$}_{\Gamma,\Delta'\cup\{(a,b)\}}\in\text{value}_\Gamma(\hat{T}_2)\). By Theorem \ref{thm:similar_variable_context} $\Delta'\cup\{(a,b)\}$ is similar to $\Delta\cup\{(a,\overline{\Gamma}(\hat{T}_1))\}$ and therefore by induction hypothesis we conclude \(\semantic{$e$}_{\Gamma,\Delta'\cup\{(a,b)\}}\in\text{value}_\Gamma(\hat{T}_2)\). Thus the conclusion holds.
