...
So-called refinement types exclude values from existing types by using a predicate (in this context also called a refinement). The definition of such a refinement can be chosen quite freely, but it's important to note that one will also need to provide some inference rules to validate such refinements. This motivates the use SMT Solvers and refinements tailored to the capabilities of specific solvers. Such a set of refinement types are for example _Liquid types_ (**L**og**i**cally **qu**al**i**fie**d** Data Types). We will now specify a version of liquid types. Note that varies definitions of liquid types vary dependent of the capability of the underlying SMT Solver. In our case we will use a very modest definition that should be provable with an arbitrary Solver.

We start by defining the syntax and semantic of valid refinements.

```{definition,name="Logical Qualifier Expressions"}
We define the set of logical qualifier expression $\mathcal{Q}$ as follows
...
\end{letIn}
We say $e$ is _well formed_ with respect to $\Theta$ iff for all variables $v$ in $e$,$\Theta(v)$ is well formed, meaning $\exists n\in\mathbb{N}.(v,n)\in\Theta$.
```

```{definition, name="Semantic of Logical Qualifier Expressions"}
\begin{letIn}
...
\mathrm{values}_\Gamma(\forall a.T) :=& \lambda b.\mathrm{values}_{\{(a,b)\}\cup\Gamma}(T) \text{ where the symbol } b \text{ does}\\
  &\text{ not occur in } T.\\
\mathrm{values}_\Gamma(\{ \nu:T \ | \ r \}) :=& \mathrm{refinedValues}_{\Gamma,\Theta}(\{ \nu:T \ | \ r \})\\
\mathrm{values}_\Gamma(a:T_1\to T_2) :=& \mathrm{refinedValues}_{\Gamma,\Theta}(a:T_1\to T_2)
\end{aligned}
$$
