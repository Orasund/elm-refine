...
$$
\begin{aligned}
\semantic{$.$}:&\mathit{IntExp}\to\mathbb{N}\\
\semantic{$n$} =& n\\
\semantic{$a + b$}=& \semantic{$a$} + \semantic{$b$}\\
\semantic{$a \cdot n$}=& \semantic{$a$} \cdot n\\
\semantic{$v$}=& v\\
\end{aligned}
$$
Note that we assume that $v$ as been bound to number.

We also define the semantic of logical qualifier expressions $\mathcal{Q}$ as follows:

$$
\begin{aligned}
\semantic{$.$}:&\mathbb{N}\to\mathit{Bool}\\
\semantic{$\mathit{True}$} =& \lambda v. \mathit{True}\\
\semantic{$\mathit{False}$} =& \lambda v.\mathit{False}\\
\semantic{$a \leq \nu$} =&  \lambda v.\semantic{$a$} \leq v\\
\semantic{$\nu < a$} =& \lambda v. \semantic{$a$} < v\\
