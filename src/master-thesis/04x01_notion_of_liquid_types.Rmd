...
$$
\begin{aligned}
\semantic{$.$}_.:&\mathcal{Q}\to(\mathcal{V}\nrightarrow\mathbb{N})\to\mathbb{N}\to\mathit{Bool}\\
\semantic{$\mathit{True}$}_\Theta =& \lambda v. \mathit{True}\\
\semantic{$\mathit{False}$}_\Theta =& \lambda v.\mathit{False}\\
\semantic{$a \leq \nu$}_\Theta =&  \lambda v.\semantic{$a$}_\Theta \leq v\\
\semantic{$\nu < a$}_\Theta =& \lambda v. v < \semantic{$a$}_\Theta\\
\semantic{$p \land q$}_\Theta =& \lambda v. \semantic{$p$}_\Theta(v) \land \semantic{$q$}_\Theta(v)\\
\semantic{$p \lor q$}_\Theta =& \lambda v. \semantic{$p$}_\Theta(v) \land \semantic{$q$}_\Theta(v)\\
\semantic{$\neg p$}_\Theta =&\lambda v. \neg \semantic{$p$}_\Theta(v)
\end{aligned}
$$
```
...
\mathrm{refinedValues}_{\Gamma,\Theta}(\{ \nu:T \ | \ r \}) :=\\
   \{ b\in \mathrm{values}_\Gamma(T) &| \ r \text{ is well formed with respect to }\Theta \land \ \semantic{$r$}_\Theta(b) \}\\
\mathrm{refinedValues}_{\Gamma,\Theta}(a:T_1\to T_2) :=\\
        \{b\in \mathrm{values}_\Gamma(T_1 \to T_2) | \ \forall n\in& \mathrm{values}_\Gamma(T_1).b(n) \in \mathrm{refinedValues}_{\Gamma,\Theta\cup\{a,n\}}(T_2)\}
\end{aligned}
$$
```