...
T \text{ is a }\mathit{liquid} \ \mathit{type} \ :\Leftrightarrow \ &\\
         & T \text{ is of form } \{ \nu :T_0 \ | \ r \}\\
         & \text{where } T_0\text{ is a type and }r\in \mathcal{Q}.\\
  \lor \ & T \text{ is of form } a:T_1\to \hat{T}_2\\
         & \text{where } T_1\in\mathcal{T} \text{, } a \text{ is a symbol}, \hat{T}_2 \text{ is a liquid type}\\
\end{aligned}
$$
```
...
\begin{aligned}
   ...
   \mathrm{free}\begin{pmatrix*}[l]
    ...
    \mathrm{free}(\{\nu :T \ | \ r \}) :=& \mathrm{free}(T)\\
    \mathrm{free}(a:T\to \hat{T}) :=& \mathrm{free}(T)\cup\mathrm{free}(\hat{T})
\end{aligned}
$$
...
$$
  \begin{aligned}[]
    ...
    [a:T\to \hat{T}]_\Theta :=& [T]_\Theta\to [\hat{T}]_\Theta
  \end{aligned}
$$
...
\mathrm{values}_\Gamma(a:T\to \hat{T}) :=& \mathrm{refinedValues}_{\Gamma,\{\}}(a:T\to \hat{T})
\end{aligned}
$$
...
\mathrm{refinedValues}_{\Gamma,\Theta}(\{ \nu:T \ | \ r \}) :=\\
   \{ t\in \mathrm{values}_\Gamma(T) &| \ r \text{ is well formed with respect to }\Theta \land \ \semantic{$r$}_\Theta(t) \}\\
\mathrm{refinedValues}_{\Gamma,\Theta}(a:T\to \hat{T}) :=\\
        \{b\in \mathrm{values}_\Gamma(T \to \hat{T}) | \ \forall n\in& \mathrm{values}_\Gamma(T).b(n) \in \mathrm{refinedValues}_{\Gamma,\Theta\cup\{a,n\}}(\hat{T})\}
\end{aligned}
$$
```