...
\begin{letIn}
$T',T\in\mathcal{T}$ and $e:\mf{<type>}$. Let $a$ be a type variable. Let $\Delta$ be a variable context. Let $A$ be a set and $\syntax{.}:\text{values}(\mf{<statement>})\cup\text{values}(\mf{<expression>})\to A$
\end{letIn}
\begin{center}
\begin{tabular}{c l}
\logicRule
...
\begin{letIn}
$T,T_1,T_2,T_3:\mathcal{T}$ Let $v:\mf{<lower-var>},e:\mf{<exp>}$ and $s:\mf{<statement>}$.
...
\Gamma,\Delta\vdash\syntax{s}:T_3
\end{gathered}
}
...
\begin{aligned}
\mf{"type"} c v_1 \dots v_n &\mf{"="} c_1 \ t_{1,1} \dots t_{1,k_1}\\
| \ \dots | \ c_m \ t_{m,1} \dots t_{m,k_m} \mf{";"} s
\end{aligned}\Bigg\rrbracket:T
}
{TCustomType}
