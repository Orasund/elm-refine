...
Types are typically inferred using so called _inference rules_ \cite{Types_and_Programming_Languages}. Using these rules we can construct a truth tree, who's leafs are axioms, to prove that $\Delta\vdash\syntax{e}:T$ for an expression $e$, a type $T$, a variable context $\Delta$ and a interpretation function for expressions $\Delta$. Such a proving process is called _type checking_. It is also possible to find the most general $T$ such that $\Delta\vdash\syntax{e}:T$ using the inference rules. This is called _type inference_.

There are two universal inference rules for any Hindley-Milner type system.
...
For the inference rules of expressions we will need to introduce a pattern matching function:
