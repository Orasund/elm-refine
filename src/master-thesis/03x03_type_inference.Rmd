...
Lets assume we can define a function

$$
\syntax{.}:\text{values}_\Gamma(\mf{<program>})\cup\text{values}_\Gamma(\mf{<expression>})\to A
$$
where $\Gamma$ is a type context and $A$ is a nonempty set. We call such a function an _interpretation_ and for a given $e\in\text{values}_\Gamma(\mf{<program>})\cup\text{values}$ we refer to \(\syntax{$e$}\) as being the _semantic_ of $e$. We will discuss the definition of such a function as well as the definition of $A$ in the next section.

Instead we are more intested in the judgment $\syntax{e}\in \text{values}_\Gamma(T)$ for a given type $T$ a type context $\Gamma$ and an expression $e$. For now lets assume that the definition of $A$ actually allows this.

### Typing Judgments

The judgment can arise from set of logical inference rules \cite{Types_and_Programming_Languages}.

```{definition,name="inference rules"}
\begin{letIn}
$n\in\mathbb{N}$, $P_i$ be a sequents for all $i\in\mathbb{N}$.
Let $C$ be a sequent.
\end{letIn}
We call
$$
...
We will see that the initial judgment is dependent on another context, this time for variables instead of types:
...
The judgment in question is now are more general case of the orignal question, namely

$$
\Delta\vdash\syntax{e}:_\Gamma T
$$
where $\Gamma$ is a type context, $\Delta$ is a variable context,  $e\in\text{values}_\Gamma(\mf{<program>})\cup\text{values}_\Gamma(\mf{<expression>})$ and $T$ is a type. Note that the values of `<program>` and `<expression>` are disjoint and therefore we will look at $e:\mf{<program>}$ and $e:\mf{<expression>}$ seperatly.

If the type $T$ is known then we talk about _type checking_ else we call the judgment a _type inference_. For infering a type, the result is not nessarly unique, that is why we might only want to find the most general type, meaning a type $T_1$ such that

$$
(\forall T_2\in\mathcal{T}\land T_1\sqsubseteq T_2.e:_\Gamma T_2) \land (\forall T_2\in\mathcal{T}\land T_2 \sqsubseteq T_1. \exists T_3\in\mathcal{T} \land T_2 \sqsubseteq T_3.\neg (e:_\Gamma T_3)).
$$
...
In the inference rules $[TConstant],[TConstant2]$ and $[Main],[Main2]$ we used a judgment for expression. We will now give the corresponding inference rules. As before, each expression has one or two rules depending on optional parameters
