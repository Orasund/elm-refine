...
In 1972 Per Martin-Löf introduced a new form of type theory, now known as the Martin-Löf type theory. Martin-Löf took the Curry-Howard-Correspondence and extended it such that it is able to write statements in predicate logic. To do so, he introduced _dependent types_ [@modern_perspective_on_type_theory].
...
While signals where a very direct implementation of the mathematical model, it turned out to be not very useful. Thus, it got replaced by a special architecture, now known as _The Elm Architecture_ (TEA) [@ElmPage]. Nowadays, an Elm program is more like a state machine with events being state transitions:
