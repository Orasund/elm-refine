...
In this section we give a quick history of type theory and the Elm langauge.
...
\begin{enumerate}
\item \mf{0} is a type;
\item If $T_1,\dots,T_n$ are simple types, then also $(T_1,\dots,T_n)$ is a simple type. For $n=0$ we obtain the simple type $()$;
\item All simple types can be constructed using rules 1 and 2.
\end{enumerate} [@modern_perspective_on_type_theory].
```

Note that $()$ stands for the type of all propositions and \mf{0} is the type of all variables. $(T_1,\dots,T_n)$ denotes the type of $n$-nary relations between values in $T_i$, for $i$ from $1$ to $n$. For example the predicate $a < b$ for variables $a,b$ would be of type $(0,0)$. For $P$ and $Q$ of type $(0)$, the predicate $P(a) \land Q(b)$ would be of type $((0))$ [@modern_perspective_on_type_theory]. The theory of types has changed a lot since then.
