...
Next, the program goes into the `Weaken`-step as starts checking each and every predicate currently associated with `kappa_1`, as seen in Figure \@ref(fig:assistant3).

```{r assistant3, echo=FALSE, fig.align='center', fig.cap="Weakening the predicates.", out.width = '75%', eval=TRUE}
knitr::include_graphics("3.png")
```

Once it has checked every predicate, it goes back to the `Solve`-step and repeats. In Figure \@ref(fig:assistant4) one can see the result after a few iterations.
