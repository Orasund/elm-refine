...
\semantic{$i \leq b$}_\Theta =&  \lambda v.\semantic{$i$}_\Theta \leq v\\
\semantic{$b < i$}_\Theta =& \lambda v. v < \semantic{$i$}_\Theta\\
...
T \text{ is a }\mathit{liquid} \ \mathit{type} \ :\Leftrightarrow \ &\\
         & T \text{ is of form } \{ a :T_0 \ | \ r \}\\
         & \text{where } T_0\text{ is a type},a \text{ is a symbol and }r\in \mathcal{Q}.\\
...
\begin{aligned}
   ...
   \mathrm{free}\begin{pmatrix*}[l]
    ...
    \mathrm{free}(\{a :T \ | \ r \}) :=& \mathrm{free}(T)\\
...
\mathrm{values}_\Gamma(\forall a.T) :=& \lambda b.\mathrm{values}_{\{(a,b)\}\cup\Gamma}(T) \text{ where the symbol } b \text{ does}\\
  &\text{ not occur in } T.\\
\mathrm{values}_\Gamma(\{ a:T \ | \ r \}) :=& \mathrm{refinedValues}_{\Gamma,\{\}}(\{ a:T \ | \ r \})\\
...
$$
\begin{aligned}
\mathrm{refinedValues}_{\Gamma,\Theta}&(\{ a:T \ | \ r \}) :=\\
   & \{ t\in \mathrm{values}_\Gamma(T) | \ \forall n\in \mathrm{values}_\Gamma(T).\\
   & r \text{ is well formed with respect to }\Theta\cup\{a,n\}\land \ \semantic{$r$}_\Theta(t) \}\\
\end{aligned}
$$
$$
\begin{aligned}
