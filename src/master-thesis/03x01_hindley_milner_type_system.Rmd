...
\begin{letIn}
$n\in\mathbb{N}_0$. Let $T := \mu C.\ | \ C_1 \ T_{1,1} \dots \ T_{1,k(1)}\ | \dots \ | \ C_n \ T_{n,1} \dots \ T_{n,k(n)}$. Let $\Gamma$ be a context.
\end{letIn}
We then have:
...
```
```{proof}
It should be clear that $\bigcup_{i\in\mathbb{N}_0^n}\mathrm{rvalues}_\Gamma(i,T)\supseteq\mathrm{rvalues}_\Gamma(n,T)$. This follows directly from the definition of the union.
In order to show $\bigcup_{i\in\mathbb{N}_0^n}\mathrm{rvalues}_\Gamma(i,T)\subseteq\mathrm{rvalues}_\Gamma(n,T)$, it is sufficient to prove by induction over $n$ that
$$
\forall n\in\mathbb{N}_0.\mathrm{rvalues}_\Gamma(n,T)\subseteq\mathrm{rvalues}_\Gamma(n+1,T).
$$
\textbf{Base case}: We will show $\mathrm{rvalues}_\Gamma(0,T)\subseteq\mathrm{rvalues}_\Gamma(1,T)$.
$$
\begin{aligned}
...
\end{aligned}
$$
\textbf{Inductive step}: Assuming $\mathrm{rvalues}_\Gamma(n,T)\subseteq\mathrm{rvalues}_\Gamma(n+1,T)$, we will prove
...
As an example we can now prove that the values of $\mathit{Nat}$ from example \ref{ex:int} are isomorphic to the natural numbers.
```{theorem}
\begin{letIn}
...
For any $n\in\mathbb{N}_0$ We define $Succ^n \ 1 := \underbrace{\mathit{Succ} \ \dots \ \mathit{Succ}}_{n \text{ times}} \ 1$. Note that $\mathit{Succ}^0 \ 1 = 1$.

Next we show by induction over $n\in\mathbb{N}_0$ that
$$\textrm{rvalues}_\Gamma(n,\mu C.1| \mathit{Succ} \ C)
=\{\mathit{Succ}^i \ 1|i\in\mathbb{N}^n_0\}.$$
...
Finally, we will be proving $$\textrm{values}(\mu C.1| \mathit{Succ} \ C)=\{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}.$$

\begin{addmargin}[1cm]{0cm}
"$\subseteq$":
Let $x\in\textrm{values}(\mu C.1| \mathit{Succ} \ C)=\bigcup_{i\in\mathbb{N}_0}\mathrm{rvalues}_\Gamma(i,\mu C.1| \mathit{Succ} \ C)$.
$$
\begin{aligned}
&x\in\bigcup_{i\in\mathbb{N}_0}\mathrm{rvalues}_\Gamma(i,\mu C.1| \mathit{Succ} \ C)\\
\Rightarrow&\exists i\in\mathbb{N}_0.x\in\mathrm{rvalues}_\Gamma(i,\mu C.1| \mathit{Succ} \ C)=\{\mathit{Succ}^k \ 1|k\in\mathbb{N}^i_0\} \\
\Rightarrow&\exists i\in\mathbb{N}_0.\exists k\in\mathbb{N}^i_0.x = \mathit{Succ}^k \ 1\\
\Rightarrow&x\in\{Succ^n \ 1|n\in\mathbb{N}_0\}
\end{aligned}
$$

"$\supseteq$":
Let $x \in\{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}$.
Then there exists $k\in\mathbb{N}_0$ such that $x = \mathit{Succ}^k \ 1$.
We know that
$$
\mathit{Succ}^k \ 1 \in \{\mathit{Succ}^i \ 1|i\in\mathbb{N}^k_0\}= \mathrm{rvalues}_\Gamma(k,\mu C.1| \mathit{Succ} \ C)\subseteq \{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}.
$$
Therefore $x = \mathit{Succ}^k \ 1 \in \{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}$.
\end{addmargin}

In conclusion, we have now shown that $\mathrm{values}(\mathit{Nat})=\{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}$. Using the bijection
$$
\begin{aligned}
&f:\{\mathit{Succ}^n \ 1|n\in\mathbb{N}_0\}\to\mathbb{N}\\
&f(\mathit{Succ}^n \ 1) = n+1\\
&f^{-1}(n) = \mathit{Succ}^{n-1} \ 1
\end{aligned}
$$
we see that $\{Succ^n \ 1|n\in\mathbb{N}_0\}\cong\mathbb{N}$.
```