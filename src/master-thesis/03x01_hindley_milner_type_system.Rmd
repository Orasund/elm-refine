...
$$
```
A poly type can be instantiated with a mono type by applying $\alpha$-Conversion and $\beta$-Reduction. The result will again be a type.
```{definition,name="Type Instatiation"}
We define
$$
  \begin{aligned}
   \text{Inst}:\ &\mathcal{T}\to(\mathcal{V}\nrightarrow\{t\in\mathcal{T}\ |\ t \text{ is a mono type}\})\to\mathcal{T}\\
   \text{Inst}(T,\Theta) :=& \begin{cases}
     \text{Inst}(T',\Theta) 
     & \text{If } T \text{ is of form } \forall a.T' \text{ and } (a,T_0)\in\Theta \text{ for } T_0\not\in\mathcal{V}\\
     & \text{where } T' \text{ is a type,} a \text{ is a variable and } T_0 \in\mathcal{T}\\
     \forall t.\text{Inst}(T',\Theta)
     & \text{If } T \text{ is of form } \forall a.T' \text{ and } (a,T_0)\in\Theta \text{ for } T_0\in\mathcal{V}\\
     & \text{where } T' \text{ is a type, } a \text{ is a variable and } T_0 \in\mathcal{T}\\
     [T]_\Theta&\text{else}
\end{cases}
  \end{aligned}
...
(\forall a_1 \dots a_n. T)(T_1,\dots,T_n) :=& \text{Apply}(\forall a_1 \dots a_n. T,\{(a_i,T_i)\ | \ i\in\mathbb{N}_0^n\})
\end{aligned}
$$
the _application constructor_ of $T$.

Therefore, for a given type $T'$, the application constructor of $T'$ is defined as $\overline{T'}$.
```
...
\begin{letIn}
$\mathcal{S}$ be the class of all finite sets,
$n\in\mathbb{N}$,
$\Theta:\mathcal{V}\nrightarrow\mathcal{T}$,
$a\in\mathcal{V}$,
$T,T_1,T_2,S\in\mathcal{T}$,
$k_j\in\mathbb{N}_0$ and
...
```{theorem}
\begin{letIn}
the algebraic type $\mathit{Nat}$ be defined as $\mathit{Nat}:=\mu C.1| \mathit{Succ} \ C$. Let $<_\mathbb{N}:\mathbb{N}\times\mathbb{N}$ be the well-order such that $a<_\mathbb{N}b :\Leftrightarrow \exists c\in\mathbb{N}. a = b + c$. Let $<_\mathit{Nat}:\mathit{Nat}\times\mathit{Nat}$ be a order such that $\underbrace{\mathit{Succ} \ \dots \ \mathit{Succ}}_{a \text{ times}} \ 1<_\mathit{Nat}\underbrace{\mathit{Succ} \ \dots \ \mathit{Succ}}_{b \text{ times}} \ 1:\Leftrightarrow a+1 <_\mathbb{N} b+1$.
\end{letIn}
Then we have:
$$(\mathrm{values}(\mathit{Nat}),<_\mathit{Nat})\cong(\mathbb{N},<_\mathbb{N})$$
```
```{proof}
We will introduce a new notation in order to simplify the proof:
