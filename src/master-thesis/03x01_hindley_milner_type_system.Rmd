...
\begin{letIn}
$n\in\mathbb{N}_0$.
\text{ Let }$a_i$ be a symbol for all $i\in\mathbb{N}_1^n$. 
\end{letIn}
We call
$$
\begin{aligned}
...
\text{values}_\Gamma\small\begin{pmatrix*}[l]
    \mu C.\\
    | C_1 \ T_{1,1} \dots \ T_{1,k(1)}\\
    | \dots \\
    | \ C_n \ T_{n,1} \dots \ T_{n,k(n)}
    \end{pmatrix*} :=&\bigcup_{i\in\mathbb{N}_0} \text{rvalues}_\Gamma
  \begin{pmatrix*}[l]
  i,&
...
\text{values}_\Gamma(\{l_1:T_1,\dots,l_n:T_n\}) :=\\
  \big\{\{l_1=t_1,&\dots,l_n=t_n\} \ | \ \forall i\in\mathbb{N}_1^n.t_i \in\text{values}_\Gamma(T_i)\big\}\\
\text{values}_\Gamma(T_1 \to T_2) :=&\{f \ | \ f:\text{values}_\Gamma(T_1)\to\text{values}_\Gamma(T_2)\}\\
...
$$
\begin{aligned}
\text{rvalues}_\Gamma(0,T):=&
  \begin{Bmatrix*}[l]
    \begin{array}{l|l}
      C_i \ v_1 \dots v_n
      & \begin{matrix*}[l]
        i\in\mathbb{N}_1^n\\
        \land \forall j\in\mathbb{N}_1^{k(i)}. T_{i,j} \neq C \land v_j \in \text{values}_\Gamma(T_{i,j})
        \end{matrix*}
    \end{array}
  \end{Bmatrix*}\\
\text{rvalues}_\Gamma({l+1},T):=&
  \small\begin{Bmatrix*}[l]
    \begin{array}{l|l}
      ...
      & \begin{matrix*}[l]
        ...
        \begin{cases}
          \text{rvalues}_\Gamma(l,T)&\text{if } T_{i,j} = C\\
          \text{values}_\Gamma(T_{i,j})&\text{else}
        \end{cases}
        \end{matrix*}
    \end{array}
  \end{Bmatrix*}
...
$$
```
The base case of this recursive function is in $\text{rvalue}(0,T)$ for a given T.

```{theorem,name="rvalue is nested"}
\begin{letIn}
$n\in\mathbb{N}_0$. Let $T := \mu C.\ | \ C_1 \ T_{1,1} \dots \ T_{1,k(1)}\ | \dots \ | \ C_n \ T_{n,1} \dots \ T_{n,k(n)}$.
\end{letIn}
$$\bigcup_{i\in\mathbb{N}_0^n} \text{rvalues}_\Gamma(i,T)=\text{rvalues}_\Gamma(n,T)
$$
```
```{proof}
Its sufficient to prove by induction over $n$ that $$\forall n\in\mathbb{N}_0.\text{rvalues}_\Gamma(n,T)\subseteq\text{rvalues}_\Gamma(n+1,T).$$
\textbf{Base case}: We'll show $\text{rvalues}_\Gamma(0,T)\subseteq\text{rvalues}_\Gamma(1,T)$.
$$
\begin{aligned}
&\text{rvalues}_\Gamma(1,T)\\
&=\small\begin{Bmatrix*}[l]
    \begin{array}{l|l}
      C_i \ v_1 \dots v_n
      & \begin{matrix*}[l]
        ...
        \land \forall j\in\mathbb{N}_1^{k(i)}. v_j \in
        \begin{cases}
          \text{rvalues}_\Gamma(0,T)&\text{if } T_{i,j} = C\\
          \text{values}_\Gamma(T_{i,j})&\text{else}
        \end{cases}
        \end{matrix*}
    \end{array}
...
\end{aligned}
$$
\textbf{Inductive step}: Assuming $\text{rvalues}_\Gamma(n,T)\subseteq\text{rvalues}_\Gamma(n+1,T)$, we'll prove
$$\text{rvalues}_\Gamma(n+1,T)\subseteq\text{rvalues}_\Gamma(n+2,T).$$
$$
  \begin{aligned}
  &\text{rvalues}_\Gamma(n+2,T)\\
  &=\small\begin{Bmatrix*}[l]
    \begin{array}{l|l}
      C_i \ v_1 \dots v_n
      & \begin{matrix*}[l]
        ...
        \land \forall j\in\mathbb{N}_1^{k(i)}. v_j \in
        \begin{cases}
          \text{rvalues}_\Gamma(n+1,T)&\text{if } T_{i,j} = C\\
          \text{values}_\Gamma(T_{i,j})&\text{else}
        \end{cases}
        \end{matrix*}
    \end{array}
  ...
  &\supseteq\small\begin{Bmatrix*}[l]
    \begin{array}{l|l}
      ...
      & \begin{matrix*}[l]
        ...
        \land \forall j\in\mathbb{N}_1^{k(i)}. v_j \in
        \begin{cases}
          \text{rvalues}_\Gamma(n,T)&\text{if } T_{i,j} = C\\
          \text{values}_\Gamma(T_{i,j})&\text{else}
        \end{cases}
        \end{matrix*}
    \end{array}
  \end{Bmatrix*}\\
  &=\text{rvalues}_\Gamma(n+1,T)
  \end{aligned}
$$
```
...
\end{letIn}
$$\text{values}(\mathit{Nat})\cong\mathbb{N}$$
```
```{proof}
$$
\begin{aligned}
\text{value}(\mu C.1| Succ \ C)&=\bigcup_{i\in\mathbb{N}_0}\text{rvalue}(i,\mu C.1| Succ \ C)\\
&=\lim_{n\to\infty}\bigcup_{i\in\mathbb{N}_0^n}\text{rvalue}(i,\mu C.1| Succ \ C)\\
&=\lim_{n\to\infty}\text{rvalue}(n,\mu C.1| \mathit{Succ} \ C).
\end{aligned}$$

We'll now show by induction over $n\in\mathbb{N}_0$ that
$$\text{rvalue}(n,\mu C.1| \mathit{Succ} \ C)
=\{1,\underbrace{\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \dots \ \mathit{Succ} \ 1}_{n \text{ times}}\}.$$

\begin{addmargin}[1cm]{0cm}
\textbf{Base case}: $rvalue(0,\mu C.1| Succ \ C) = \{1\}$\newline
\textbf{Inductive step}:\newline
Assuming $\text{rvalue}(n,\mu C.1| \mathit{Succ} \ C)
=\{1,\underbrace{\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \dots \ \mathit{Succ} \ 1}_{n \text{ times}}\},$
we'll prove $\text{rvalue}(n+1,\mu C.1| \mathit{Succ} \ C)
=\{1,\underbrace{\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \dots \ \mathit{Succ} \ 1}_{n+1 \text{ times}}\}.$
  
$$
\begin{aligned}
&\text{rvalue}(n+1,\mu C.1| \mathit{Succ} \ C)\\
&=\small\begin{Bmatrix*}[l]
    \begin{array}{l|l}
      C_i \ v_1 \dots v_n
      & \begin{matrix*}[l]
...
$$
\end{addmargin}
We define a order on $$\text{value}(\mathit{Nat})=\lim_{n\to\infty}\{1,\underbrace{\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \dots \ \mathit{Succ} \ 1}_{n \text{ times}}\}=\{1,\mathit{Succ} \ 1,\dots,\mathit{Succ} \ \mathit{Succ} \ 1,\dots\}$$
by $v_1 < v_2 :\Leftrightarrow v_2 = \mathit{Succ} \dots \mathit{Succ} \ v_1$ for $v_1,v_2\in\text{value}(\mathit{Nat})$.
