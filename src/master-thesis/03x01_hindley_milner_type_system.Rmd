...
* $\mathcal{V} := \{a| a \text{ is a symbol} \}$ as the set of all type variables(symobls).
...
$\forall a_1 \dots a_n$. $T$ be a poly type and $T$ be a mono type.
\end{letIn}
We call
$$
\begin{aligned}
f :& \mathcal{T} \to \dots \to \mathcal{T} \to \mathcal{T}\\
f(T_1,\dots,T_n) :=& [ \forall a_1 \dots a_n. T]_{(a_1,T_1),\dots,(a_n,T_n)}
\end{aligned}
$$
...
\begin{aligned}
  &\Gamma = \{\}\\
  \lor& \ \exists a. a \text{ is a type variable}\land \Gamma = \Delta \cup \{(a,T)\} \text{ where } T \text{ is a mono type and } \Delta \text{ is a}\\  
      &\text{context}.\\
  \lor& \ \exists T . T \text{is a type application} \land \Gamma = \Delta \cup \{(C,f)\} \text{ where } T = C \ T_1 \dots T_n, T_i \text{ are}\\
      & \text{mono types for } i\in\mathbb{N}_1^n\text{ and }f\text{ is a application constructor such that } f(T_1,\dots,T_n)\\
      &\text{ is a mono type}.
\end{aligned}
$$
...
$$
\begin{aligned}
\text{value}_\Gamma &: \mathcal{V} \to \mathcal{S}\\
\text{value}_\Gamma(a) &:= \Gamma(a)\\
\text{value}_\Gamma(C \ T_1 \ \dots \ T_n) &:= \Gamma(C)(T_1,\dots,T_n)\\
\text{value}_\Gamma\small\begin{pmatrix*}[l]
C_1 \ T_{1,1} \dots \ T_{1,k(1)}\\
    | \dots \\
    | \ C_n \ T_{n,1} \dots \ T_{n,k(n)}
    \end{pmatrix*} &:=\\
  \{C_i \ t_{i,1}& \dots \ t_{i,k(1)} \ | \ \forall i\in\mathbb{N}_1^n,j\in\mathbb{N}_1^{k(i)}. t_{i,j}\in\text{value}_\Gamma(T_{i,j})\}\\
\text{value}_\Gamma(\{l_1:T_1,\dots,l_n:T_n\}) &:=\big\{\{l_1=t_1,\dots,l_n=t_n\} \ | \ \forall i\in\mathbb{N}_1^n.t_i \in\text{value}_\Gamma(T_i)\big\}\\
\text{value}_\Gamma(T_1 \to T_2) &:=\{f \ | \ f:\text{value}_\Gamma(T_1)\to\text{value}_\Gamma(T_2)\}\\
\text{value}_\Gamma(\forall a.T) &:= \lambda b.\text{value}_{\{(a,b)\}\cup\Gamma}(T) \text{ where the symbol } b \text{ does not occur in } T.
\end{aligned}
$$
```
