...
&\begin{aligned} \Theta':= \{\ (&a,r)\\
   | \ &r = q \land (a,q)\in\Theta \land q\in\mathcal{Q}\\
\lor \ &r = [[k]_S]_{S_0}\land (a,q)\in\Theta\land q=[k]_{S_0} \land k\in\mathcal{K} \land S_0\in\mathcal{V}\nrightarrow\mathcal{Q}\}
...
\begin{aligned}
&\neg (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow \semantic{$r_2$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}})\\
\end{aligned}
$$
by solving the following SMT statement:
$$
\exists i_1\in\mathbb{N}.\dots\exists i_n\in\mathbb{N}.\semantic{$(\bigwedge_{j=0}^n r_j')\land r_1\land p\land \neg r_2$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}
$$
Note that \(\semantic{$q$}\) translates the refinement $q\in\mathcal{Q}$ into a SMT-readable statement.
...
&\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow\semantic{$[q]_{S_2}$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}})\}
\end{aligned}\end{aligned}\\
...
\end{aligned}
$$
To do so, we first need to compute $r_2 := [q]_{S_2}$, with that we can now formulate our SMT statement:
$$
\forall i_1 \in\mathbb{N}.\dots \forall i_n \in\mathbb{N}.\semantic{$\neg((\bigwedge_{j=0}^n r_j')\land r_1\land p)\lor r_2$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}
$$
