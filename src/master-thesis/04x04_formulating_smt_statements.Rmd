...
&\quad\quad\text{Split}(\hat{T}_3 <:_{\Theta,\Lambda}\hat{T}_1)\cup\text{Split}(\hat{T}_2 <:_{\Theta\cup\{(a,q_3)\},\Lambda}\hat{T}_4\})\\
&\text{Split}(\{\nu:\mathit{Int}|q_1\}<:_{\Theta,\Lambda}\{\nu:\mathit{Int}|q_2\} )=\\
&\quad\quad\{ \{\nu:\mathit{Int}|q_1\}<:_{\Theta,\Lambda}\{\nu:\mathit{Int}|q_2\} \}
\end{aligned}
$$
...
&r_1 := \begin{cases}\bigwedge [S(k_1)]_{S_1}&\text{ if } x \text{ has the form } [k_1]_{S_1}\\
x&\text{ if } x\in\mathcal{Q}
\end{cases},\\
...
&\quad\text{ such that } \hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2 \text{ is wellformed.}
\end{aligned}
$$
...
\begin{letIn}
$C$ be a set of wellformed conditions, $S := \text{Infer}(C)$ and $V:=\bigcup_{\hat{T}_1<:_{\Theta,\Lambda}\hat{T}_2\in C}\{a \ | \ (a,\_)\in\Theta\}$
\end{letIn}
...
\land\ &\forall S'\in(\mathcal{V}\to\mathcal{Q}).(\forall a.\exists Q\in\mathit{Init}(V). S'(a) = \bigwedge Q)\\
&\quad\land [\hat{T}_1]_{S'} \in\mathcal{T} \land [\hat{T}_2]_{S'} \in\mathcal{T}  \land ([\hat{T}_1]_{S'} <:_{\Theta,\Lambda} [\hat{T}_2]_{S'} \Rightarrow (\forall a. S(a) \Rightarrow S'(a)))\end{aligned}
$$
```
The first post-condition states that $[\hat{T}_1]_S$ and $[\hat{T}_2]_S$ are not templates.
The second condition states that $[\hat{T}_1]_S$ is a subtype of $[\hat{T}_2]_S$.
The third condition states that $S$ is the sharpest solution.

```{theorem}
\label{thm:correctness_split}
\begin{letIn}
$c$ is a wellformed condition and $C := \text{Split}(c)$.
\end{letIn}
Then $C\subseteq \mathcal{C^-} \land \forall c\in C. c$ is a wellformed condition.
```
...
\land&[\hat{T}_1]_S <:_{\Theta,\Lambda} [\hat{T}_2]_S\\
\land&\forall S'\in(\mathcal{V}\to\mathcal{Q}).(\forall a.\exists Q\in\mathit{Init}(V). S'(a) = \bigwedge Q)\\
&\quad\land [\hat{T}_1]_{S'} \in\mathcal{T} \land [\hat{T}_2]_{S'} \in\mathcal{T}  \land ([\hat{T}_1]_{S'} <:_{\Theta,\Lambda} [\hat{T}_2]_{S'} \Rightarrow (\forall a. S(a) \Rightarrow S'(a)))
\end{aligned}
$$
```
...
\begin{letIn}
$c\in\mathcal{C}^-$ be a well-formed condition and $A_1,A_2:\mathcal{K}\nrightarrow\mathcal{Q}$. Let $A_2 = \text{weaken}(c,A_1)$
\end{letIn}

if $c$ has the form $\{\nu:\mathit{Int}|[k_1]_{S_1}\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$ then 
$$
\begin{aligned}
&(\forall k\neq k_2. A_1(k) = A_2(k))\\
\land\ &A_2(k_2)\subseteq A_1(k_2)\\
\land\ &\text{Let } S := \{(k,\bigwedge Q)| (k,Q)\in A_2\}\text{ in}\\
&[k_1]_S\in\mathcal{Q}\land[k_2]_S\in\mathcal{Q}\land \{\nu:\mathit{Int}|[[k_1]_S]_{S_1}\}<:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[[k_2]_S]_{S_2}\}\\
\land\ &\forall A_2':\mathcal{V}\nrightarrow\mathcal{P}(\mathcal{Q}).(\forall k\neq k_2. A_1(k) = A_2'(k))\\
  &\land\ A_2'(k_2)\subseteq A_1(k_2)\\
  &\land \text{Let } S' := \{(k,\bigwedge Q)| (k,Q)\in A_2'\}\text{ in}\\
  &\quad [k_1]_{S'}\in\mathcal{Q}\land  [k_2]_{S'}\in\mathcal{Q}\land\{\nu:\mathit{Int}|[[k_1]_{S'}]_{S_1}\}<:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[[k_2]_{S'}]_{S_2}\}\\
  &\quad \Rightarrow A_2\subseteq A_2'
\end{aligned}
$$
...
\land\ &A_2(k_2)\subseteq A_1(k_2)\\
\land\ &\text{Let } S := \{(k,\bigwedge Q)| (k,Q)\in A_2\}\text{ in}\\
&[k_2]_S\in\mathcal{Q}\land \{\nu:\mathit{Int}|r\}<:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[[k_2]_S]_{S_2}\}\\
\land\ &\forall A_2':\mathcal{V}\nrightarrow\mathcal{P}(\mathcal{Q}).(\forall k\neq k_2. A_1(k) = A_2'(k))\\
  &\land\ A_2'(k_2)\subseteq A_1(k_2)\\
  &\land \text{Let } S' := \{(k,\bigwedge Q)| (k,Q)\in A_2'\}\text{ in}\\
  &\quad [k_2]_{S'}\in\mathcal{Q}\land \{\nu:\mathit{Int}|r\}<:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[[k_2]_{S'}]_{S_2}\}\\
  &\quad \Rightarrow A_2\subseteq A_2'
\end{aligned}
$$
```
The first post-condition state that by updating $A_1$ to $A_2$ only the value for $k_2$ changes. The second condition states that the updated value $A_2(k_2)$ needs to be a subset of the old value $A_1(k_2)$. The third condition states that the resulting substitution $S$ generates a refinement $[k_1]_S$ that makes the subtyping condition valid. The fourth condition states that the value $A_2(k_2)$ is the smallest subset of $A_1(k_2)$ such that the previous conditions hold.