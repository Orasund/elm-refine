...
&\text{where }\hat{T}_1,\hat{T}_2\text{ are a liquid types or templates},\Theta\text{ is a type variable context and }\\
...
\begin{aligned}
  ...
  &\text{Vars}(\hat{T}_1)\cup\text{Vars}(\hat{T}_2)\\
...
&\quad\quad\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow \semantic{$r_2$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}})\\
...
\begin{aligned}
&\neg (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow \semantic{$r_2$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}})\\
\end{aligned}
$$
by deciding the satisfiablity of
$$(\bigwedge_{j=0}^n r_j')\land r_1\land p\land \neg r_2$$
...
\land & (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow\semantic{$[q]_{S_2}$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}})\}
...
$$\neg((\bigwedge_{j=0}^n r_j')\land r_1\land p)\lor r_2
$$
with free variables $b_i\in\mathbb{N}$ for $i\in\mathbb{N}_1^n$.
...
```{definition,name="Wellformed Subtyping Condition"}
...
\lor\ &c \text{ has the form } \{\nu:\mathit{Int}|[k_1]_{S_1}\}\to \hat{T}_1 <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|r\}\to \hat{T}_2\\
&\quad\text{ such that } \hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2 \text{ is well-formed.}
\end{aligned}
$$
where $r\in\mathcal{Q}$,$k_1,k_2\in\mathcal{K}$, $S_1,S_2:\mathcal{V}\nrightarrow\mathcal{Q}$,$\Theta$ is a type variable context, $\Lambda\subset\mathcal{Q}$ and $T_1,T_2\in(\mathcal{T}\cup\mathcal{T}^?)$
```

```{theorem}
\label{thm:correctness_infer}
\begin{letIn}
$C$ be a set of well-formed conditions, $S := \text{Infer}(C)$ and $V:=\bigcup_{\hat{T}_1<:_{\Theta,\Lambda}\hat{T}_2\in C}\{a \ | \ (a,\_)\in\Theta\}$
\end{letIn}
For all subtyping condition $(\hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2)\in C$, let 
$$
\begin{aligned}
& \begin{aligned} \Theta':= \{\ (&a,r)\\
   | \ &r \text{ has the form } q \land (a,q)\in\Theta \land q\in\mathcal{Q}\\
...
&\quad\land q \text{ has the form } [k]_{S_0} \land k\in\mathcal{K} \land S_0\in\mathcal{V}\nrightarrow\mathcal{Q}\}
\end{aligned}\\
&\{(b_1,r_1'),\dots,(b_n,r_n')\}=\Theta'\end{aligned}
$$
$$
\begin{aligned}
&\text{in }[\hat{T}_1]_S \in\mathcal{T} \land [\hat{T}_2]_S \in\mathcal{T}\\
&\land \ [\hat{T}_1]_S <:_{\Theta',\Lambda} [\hat{T}_2]_S\\
...
&\quad\land [\hat{T}_1]_{S'} \in\mathcal{T} \land [\hat{T}_2]_{S'} \in\mathcal{T}\\
&\quad\land ([\hat{T}_1]_{S'} <:_{\Theta',\Lambda} [\hat{T}_2]_{S'}\\
&\quad\quad \Rightarrow (\forall a\in\mathcal{V}.S(a) \text{ and } S'(a) \text{ are well defined}\\ &\quad\quad\quad \Rightarrow (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\quad\quad\quad\semantic{$S(a)$}_{(b_1,i_1),\dots,(b_n,i_n)} \Rightarrow \semantic{$S'(a)$}_{(b_1,i_1),\dots,(b_n,i_n)})))\end{aligned}
$$
```
The first post-condition states that $[\hat{T}_1]_S$ and $[\hat{T}_2]_S$ are not templates.
The second condition states that $S$ is a solution, meaning that $[\hat{T}_1]_S$ is a subtype of $[\hat{T}_2]_S$.
The third condition states that $S$ is the sharpest solution.
```{proof}
Let $C$ be a set of well-formed conditions, $S := \text{Infer}(C)$ and $V:=\bigcup_{\hat{T}_1<:_{\Theta,\Lambda}\hat{T}_2\in C}\{a \ | \ (a,\_)\in\Theta\}$.
Let $Q_0:=\mathit{Init}(V)$,
$A_0:=\{(\kappa,Q_0)\ | \ \kappa\in\bigcup_{c\in C}\text{Var}(c)\}$ and $C_1:= \bigcup_{c\in C}\text{Split}(c)$. Then by Theorem \ref{thm:correctness_split} $C_1\subset\mathcal{C}^-$ is a set of well-formed condition. Let $A:=\text{Solve}(C_1,A_0)$ and therefore $S=\{(\kappa,\bigwedge Q) \ | \ (\kappa,Q)\in A\}$. Then by Theorem \ref{thm:correctness_solve} the conclusion holds.
```

```{theorem}
\label{thm:correctness_lemma}
\begin{letIn}
$c\in\mathcal{C}^-$ be a well-formed condition.
\end{letIn}
if and only if
$$
\begin{aligned}
...
\end{aligned}
$$
where $r\in\mathcal{Q}$,$k_1,k_2\in\mathcal{K}$, $S:\mathcal{V}\nrightarrow\mathcal{Q}$,$\Theta$ is a type variable context and $\Lambda\subseteq\mathcal{Q}$.
```
This theorem will be used to break down the statement "$c\in\mathcal{C}^-$ is a well-formed condition" into two cases. Both directions will be used.
```{proof}
"$\Rightarrow$": Let $c\in\mathcal{C}^-$ be a well-formed condition.
We make a case distinction for well-formed conditions:
  
\textbf{Case} $c = \{\nu:\mathit{Int}|[k_1]_{S_1}\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$: Then the conclusion holds.

\textbf{Case} $c = \{\nu:\mathit{Int}|r\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$: Then the conclusion holds.

\textbf{Case} $c = \{\nu:\mathit{Int}|[k_1]_{S_1}\}\to \hat{T}_1 <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|r\}\to \hat{T}_2$ such that $\hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2$ is well-formed.: Then $c\not\in\mathcal{C}^-$ and therefore this is an empty case and the conclusion trivially holds.
...
\end{letIn}
Then $C\subseteq \mathcal{C^-} \land \forall c\in C. c$ is a well-formed condition.
```
```{proof}
Let $c$ is a well-formed condition. \text{Split}(c) is therefore defined and $C := \text{Split}(c)$. 

\textbf{Case} c = $\{\nu:\mathit{Int}|q\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$:
  Then $C = {c}$ and by appling Theorem \ref{thm:correctness_lemma} to $c$ the conclusion holds.

\textbf{Case} c = $\{\nu:\mathit{Int}|[k_1]_{S_1}\}\to \hat{T}_1 <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|r\}\to \hat{T}_2$ such that $\hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2$ is well-formed: Let $C_0:=\text{Split}(\hat{T}_1<:_{\Theta,\Lambda} \hat{T}_2)$. By the induction hypothesis $C_0\subseteq \mathcal{C^-} \land \forall c\in C_0. c$ is a well-formed condition and therefore by appling Theorem \ref{thm:correctness_lemma} to each element in $C :=\{\{\nu:\mathit{Int}|r\}<:_{\Theta,\Lambda}\{\nu:\mathit{Int}|[k_1]_{S_1}\}\}\cup C_0$ the conlusion holds.
```
...
\begin{letIn}
$C\subseteq\mathcal{C}^-$ be a set of well-formed conditions, $A_1,A_2:\mathcal{K}\nrightarrow\mathcal{Q}$ and $V:=\bigcup_{\hat{T}_1<:_{\Theta,\Lambda}\hat{T}_2\in C}\{a \ | \ (a,\_)\in\Theta\}$. Let for all $a\in V$, $A_1(a)$ be well defined. Let $A_2 = \text{Solve}(C,A_1)$ and
...
&\text{in }[\hat{T}_1]_S \in\mathcal{T} \land [\hat{T}_2]_S \in\mathcal{T}\\
&\land\ [\hat{T}_1]_S <:_{\Theta',\Lambda} [\hat{T}_2]_S\\
&\land\ \forall S'\in(\mathcal{V}\to\mathcal{Q}).(\forall a\in\mathcal{V}.S'(a) \text{ well defined} \Rightarrow \exists Q\in\mathit{Init}(V). S'(a) = \bigwedge Q)\\
&\quad\land [\hat{T}_1]_{S'} \in\mathcal{T} \land [\hat{T}_2]_{S'} \in\mathcal{T}\\
&\quad\land ([\hat{T}_1]_{S'} <:_{\Theta',\Lambda} [\hat{T}_2]_{S'}\\
&\quad\quad\Rightarrow (\forall a\in\mathcal{V}.S(a)\text{ and } S'(a) \text{ are well defined}\\
&\quad\quad\quad\Rightarrow (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\quad\quad\quad\semantic{$S(a)$}_{(b_1,i_1),\dots,(b_n,i_n)} \Rightarrow \semantic{$S'(a)$}_{(b_1,i_1),\dots,(b_n,i_n)})))
\end{aligned}
$$
```
...
\textbf{Show that the precondition of} $\text{Solve}(C,\text{Weaken}(c,A))$ \textbf{holds.} 

By Theorem \ref{thm:correctness_weaken} $A_2(a)$ is defined for all $a$ for which $A_1$ is defined, namely for all $a\in V$, thus the precondition holds.

\textbf{Show the else-branch ensures the postcondition}

Once the recursion is done, we can assume that for all $c\in C$, By Theorem \ref{thm:correctness_lemma} $c$ has the form $(\{\nu:\mathit{Int}|q_1\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\})$ and the following holds.
...
\end{aligned}
$$
where $k,k_2\in\mathcal{K},S:\mathcal{K}\nrightarrow\mathcal{Q},Q,\Lambda\subseteq\mathcal{Q},S_1,S_2:\mathcal{V}\nrightarrow\mathcal{Q},q_1\in\mathcal{K}\cup\mathcal{Q}, \Theta$ be a type variable context $,r_1,p,r_2\in\mathcal{Q},a\in\mathcal{V},\Theta':\mathcal{V}\nrightarrow\mathcal{Q},r\in\mathcal{Q},n\in\mathbb{N}, b_i\in\mathcal{V},r_i\in\mathcal{Q}$ for $i\in\mathbb{N}_0^n$ and $[t]_A$ denotes the substitution for the term $t$ with a substitution $A$.
This is the same as saying $[\hat{T}_1]_S <:_{\Theta',\Lambda} [\hat{T}_2]_S$ if $[\hat{T}_1]_S \in\mathcal{T}$ and $[\hat{T}_2]_S \in\mathcal{T}$. We know by Theorem \ref{thm:correctness_weaken} that $A_2(a)$ is defined for all $a\in V$ and thus
$[\hat{T}_1]_S \in\mathcal{T} \land [\hat{T}_2]_S \in\mathcal{T}$.

We still need to show that it is the sharpest solution. To do so, we will use prove by contradiction. Assume there exists S' and $a\in V$ such that $S'(a)$ is sharper then $S(a)$. If $S(a) = \mathit{False}$ then there can not exist a sharper refinement $S'(a)$ and therefore this is a contradiction. If $S(a) \neq \mathit{False}$ then it must have been produced by calling Weaken. This is a contradiction, as by Theorem \ref{thm:correctness_weaken} we know that Weaken produces the sharpest solution therefore $S'(a) = S(a)$. 
```

```{theorem}
\label{thm:correctness_weaken}
\begin{letIn}
$c\in\mathcal{C}^-$ be a well-formed condition and therefore by Theorem \ref{thm:correctness_lemma} has the form $$\{\nu:\mathit{Int}|x\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$$ where $x$ has the form $[k_1]_{S_1}$ or $r$. Let $A_1,A_2:\mathcal{K}\nrightarrow\mathcal{Q}$. Let for all $a\in V$, $A_1(a)$ be well defined and $A_2 = \text{Weaken}(c,A_1)$. Let 
$$
\begin{aligned}
...
\land\ &\forall A_2':\mathcal{V}\nrightarrow\mathcal{P}(\mathcal{Q}).(\forall k\neq k_2. A_1(k) = A_2'(k))\\
  ...
  &\quad\quad r_1' := \begin{cases}\bigwedge [S(k_1)]_{S_1}&\text{if } q_1 \text{ has the form } [k_1]_{S_1} \text{ for } k\in\mathcal{K}\text{ and } S_1\in\mathcal{V}\nrightarrow\mathcal{Q}\\
q_1& \text{if }q_1\in\mathcal{Q}
\end{cases}\\
...
```{proof}
Let $c\in\mathcal{C}^-$ be a well-formed condition and therefore by Theorem \ref{thm:correctness_lemma} $c$ has the form $\{\nu:\mathit{Int}|x\} <:_{\Theta,\Lambda} \{\nu:\mathit{Int}|[k_2]_{S_2}\}$ where $x$ has the form $[k_1]_{S_1}$ or $r$. Let $A_1,A_2:\mathcal{K}\nrightarrow\mathcal{Q}$. Let for all $a\in V$, $A_1(a)$ be well defined and $A_2 = \text{Weaken}(c,A_1)$. Let 
$$
\begin{aligned}
  &S := \{(k,\bigwedge Q)| (k,Q)\in A_2\}\\
&r_1 := \begin{cases}\bigwedge [S(k_1)]_{S_1}&\text{if } q_1 \text{ has the form } [k_1]_{S_1} \text{ for } k\in\mathcal{K}\text{ and } S_1\in\mathcal{V}\nrightarrow\mathcal{Q}\\
...
We know $A_2(k_2) = Q_2$ and $Q_2\subseteq A_1(k_2)$ therefore $A_2(k_2)\subseteq A_1(k_2)$.
...
We finish off by proving that the result is the sharpest, meaning that $A_2$ is the smallest subset such hat the premise holds. So let there exists a $A_2'$ such that $A_2'(k_2)\subseteq A_2(k_2)$. This means there exists a $q\in A_2'$ such that 
$$
\begin{aligned}
&q\in A(k_2)\land \text{wellFormed}(q,\{(b_1,\{\nu:\mathit{Int}|r_1'\}),\dots,(b_n,\{\nu:\mathit{Int}|r_n'\})\})\\
&\quad\land (\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\quad\semantic{$r_1\land p$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}\Rightarrow\semantic{$[q]_{S_2}$}_{\{(b_1,i_1),\dots,(b_n,i_n)\}}).
\end{aligned}
$$
By the definition of $A_2$, this means that $q\in A_2$. Thus $A_2$ is the smallest subset.
```