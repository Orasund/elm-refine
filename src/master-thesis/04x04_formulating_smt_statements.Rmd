...
\end{letIn}
For every subtyping condition $(\hat{T}_1 <:_{\Theta,\Lambda} \hat{T}_2)\in C$, let 
$$
\begin{aligned} \Theta':= \{\ (&a,r)\\
   | \ &r \text{ has the form } q \land (a,q)\in\Theta \land q\in\mathcal{Q}\\
\lor \ &r \text{ has the form } [[k]_S]_{S_0}\land (a,q)\in\Theta\\
...
&\quad\land [\hat{T}_1]_{S'} \in\mathcal{T} \land [\hat{T}_2]_{S'} \in\mathcal{T}\\
&\quad\land ([\hat{T}_1]_{S'} <:_{\Theta',\Lambda} [\hat{T}_2]_{S'}\\
&\quad\quad\Rightarrow \forall a\in V.\forall i_1\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_1'\}).\dots \forall i_n\in\text{value}_\Gamma(\{\nu:\mathit{Int}|r_n'\}).\\
&\quad\quad\quad\semantic{$S(a)$}_{(b_1,i_1),\dots,(b_n,i_n)} \Rightarrow \semantic{$S'(a)$}_{(b_1,i_1),\dots,(b_n,i_n)})
\end{aligned}
$$
```
...
\textbf{Show that the precondition of} $\text{Solve}(C,\text{Weaken}(c,A_1))$ \textbf{holds.} 
...
We know that $A_2 = \text{Solve}(C,\text{Weaken}(c,A_1))$.  By the postcondition of $\text{Solve}(C,\text{Weaken}(c,A_1))$, we know that for every $a\in V$, $A_2(a) \subseteq \text{Weaken}(c,A_1)$. By Theorem \ref{thm:correctness_weaken} we also know that  for every $a\in V$,
