...
Elm differentiates variables depending on the capitalization of the first letter. For the formal language we define `<upper-letter>`$\in\mathcal{V}$ for variables with the first letter capitalized and `<lower-var>`$\in\mathcal{V}$ for variables without.
...
\begin{letIn}
  \mf{<upper-letter>}\in\mathcal{V};\quad
  \mf{<lower-var>}\in\mathcal{V}.
\end{letIn}
