...
\begin{itemize}
\item \textbf{Case} $s = \mathit{mss} \ a \ \mf{"="} \ e$ for $\mathit{mss}\in\mf{<maybe-statement-sort>}$, $a \in\mathcal{V}$, $e\in\mf{<exp>}$, $\Gamma_1=\Gamma_2$ and $\Delta_2=\text{insert}_{\Delta_1}(\{(a,T)\})$ for $T\in\mathcal{T}$: Then from the premise of the inference rule, we assume that $\Gamma_1,\mathit{mss}\vdash e:T$ and $\Gamma_1,\Delta_2\vdash e:T$ can both be derived. Then by Theorem \ref{thm:soundness_exp}, we know $\semantic{$e$}_{\Gamma_1,\Delta_1'}\in\text{value}_\Gamma(T)$. Let \(\Delta_2'=\Delta_1'\cup\{(a,\semantic{$e$}_{\Gamma_1,\Delta_1'})\}\). Then \(\semantic{$s$}(\Gamma_1,\Delta_1')=(\Gamma_2,\Delta_2')\).

We will now show that $\Delta_2'$ is similar to $\Delta_2$. We know $\Delta_1$ is similar to $\Delta_1'$ with respect to $\Gamma$, meaning $\forall T \in\mathcal{T}.\forall a \in\mathcal{V}. (a,T) \in \Delta_1 \Rightarrow \exists e \in \text{value}_\Gamma(T). (a,e) \in \Delta_1'$. Let $\a'\in\mathcal{V}$ and $T'\in\mathcal{T}$ such that $(a',T')\in\Delta_2$. We already know $\Delta_2=\text{insert}_{\Delta_1}\{(a,T)\}=\Delta_1 \cup \{(a,T'')\}$ for $T''\in\mathcal{T}$ such that $T''$ is the polytype one obtains by binding all free variables in $T$.
\begin{itemize}
\item \textbf{Case} $(a',T')\in\Delta_1$: Because $\Delta_1$ is similar to $\Delta_1'$ we can directly conclude $\exists e \in \text{value}_\Gamma(T). (a,e) \in \Delta_1'$.
\item \textbf{Case} $(a',T')\in\{(a,T'')\}$: We clame (//TODO: proof) that the semantics only produce bound variable types. Thus $T'=T''=T$ and by $\semantic{$e$}_{\Gamma_1,\Delta_1'}\in\text{value}_\Gamma(T)$ we conclude \((a,\semantic{$e$}_{\Gamma_1,\Delta_1'}) \in \Delta_2'\).
\end{itemize}
and therefore from  we can follow directly that . Thus the conclusion holds.
