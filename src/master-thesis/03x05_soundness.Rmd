...
```{proof}
\begin{itemize}
\item\textbf{Case} $\mathit{lef}=a \ \mf{"="} \ e$ for $a\in\mathcal{V}$ and $e\in\mf{<type>}$: Assume $\Gamma,\Delta\vdash\mathit{lef}:T$ then $T = \{a:T_1\}$ and $\Gamma,\Delta\vdash e:T_1$ for $T_1\in\mathcal{T}$. By induction hypothesis $\semantic_\Gamma{e}\in\text{value}_\Gamma(T)$
\end{itemize}
```
