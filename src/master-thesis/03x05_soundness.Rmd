...
\begin{letIn}
$\Gamma$ be a type context,$\mathit{lt}\in\mf{<list-type>}, T_i\in\mathcal{T}$ for $i\in\mathbb{N}_1^n$ and $n\in\mathbb{N}_0$. Let $\Gamma\vdash\mathit{lt}:(T_1,\dots,T_n)$ hold. Assume for all occurences $t\in\mf{<type>}$ in $\mathit{lt}$ there exists $T\in\mathcal{T}$ such that if $\Gamma\vdash t:T$ holds then \(\semantic{$t$}_\Gamma=T\).
\end{letIn}
