...
In 1902, Bertrand Russell wrote Gottlob Frege a letter, pointing out a paradox in Gottlob's definition of sets in his paper _Begriffsschrift_ [@Frege]. Up until this point mathematicians expected that the naive theory of sets is well formed. Russell's paradox gave a contradiction to this assumption:
...
In Russells original _ramified theory of types_ defined a order amongst the predicates. The lowest predicates could only reason about types. Higher predicates could reason only about lower predicates. This made the type of functions not only depend on the types of its arguments but also on the types of predicates contained in the definition of the function. Thus the type of a functions could get very complicated even for simple functions [@modern_perspective_on_type_theory].

In 1921 Leon Chwistek and Frank Ramsey noticed that if one would allow recursive type definitions, the complexity could be avoided. This new theory is general referred as _simple type theory_. It does as follows

```{definition,name="sime type theory"}
\begin{enumerate}
\item \mf{0} is a type;
\item If $t_1,\dots,t_n$ are simple types, then also $(t_1,\dots,t_n)$ is a simple type, $n=0$ is allowed: then we obtain the simple type $()$;
\item All simple types can be constructed using rules 1 and 2.
\end{enumerate}
We use $t,u,t_1,...$ as metavariables over simple types [@modern_perspective_on_type_theory].
```

$()$ stands for the type of the propositions and the type of the individuals [@modern_perspective_on_type_theory].
