...
\begin{letIn} `<upper-var>` and `<lower-var>` be two variable domains and $\mf{<int>}$ as in Section \@ref(syntax) definined.
\end{letIn}
...
\land \ &\text{wellFormed}(\hat{T},\Theta\cup\{(a,\{b:\mathit{Int}\ |\ r \})\})
\end{aligned}
\]
```
...
There are some liquid types that hows values are a subset of the values from another type. In this case we say it is a subtype. For our usecase we will use a different definition of subtyping.
...
a_1:\{b_1:\mathit{Int}| q_1 \}\to\hat{T}_2 &<:_{\Theta,\Lambda} a_1:\{ b_2 : \mathit{Int}| q_2\}\to\hat{T}_4 \ :\Leftrightarrow\\ 
...
By using a liquid type we can avoid dividing by zero.
