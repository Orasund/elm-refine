...
We will also extend the inference rules. The interesting part is the new judgment for \mf{<exp>}: We introduce two new set: $\Theta$ and $\Lambda$.
As before, $\Theta$ will contain the type of a variable (similarly to the previous section where we $\Theta$ stored the value of a variable).  $\Lambda$ contains boolean expressions that get collected while traversing if-then-else branches. We will use these expressions to allow path sensitive subtyping.
...
which can be read as "$\mathit{iet}$ corresponds $\mathit{exp}$".
...
&(\ \forall n\in\mathbb{N}.\forall e \in\Lambda.\\
   ...
   &\quad\land \semantic{$r_1$}_{\{(a_1,n),(b_1,k_1),\dots,(b_n,k_n)\}} \land \semantic{$r$}\\
                                                                                  &)\Rightarrow\forall n\in\mathbb{N}.\semantic{$r_2$}_{\Theta\cup\{(a_2,n)\}}\\
a_1:\hat{T}_1\to\hat{T}_2 <:_{\Theta,\Lambda} a_2:\hat{T}_3\to\hat{T}_4 \ :\Leftrightarrow \ &
  \forall n\in\text{value}_{\{\}}(\hat{T}_3).\\
&\hat{T}_1<:_{\Theta,\Lambda}\hat{T}_3 \land \hat{T}_2<:_{\Theta\cup\{(a_1,n)\},\Lambda}\hat{T}_4
\end{aligned}
$$
...
Note that we assume that $e_1\in\mf{<qualifier-type>}$. If this is not the case, then the judgment can't be derived.
