...
```{definition,name="Type variable, Mono types, Poly types"}
We say
...
where $T_1, T_2, T_i, T_{i,j}$ are a mono types, $n\in\mathbb{N}$, 
$k\in\mathbb{N}_1^n\to\mathbb{N}_0$, $l_i,C_i$ are symbols for all $i\in\mathbb{N}_1^n$, $j\in\mathbb{N}_0^{k_i}$, $T'$ is a type and $a$ is a symbol in $T'$.

&nbsp;
```

```{block2, type="myexample",latex.options="_"}
The symbol `string` is a type variable. It can be though of as a type, whose implementation is unknown. For real programming languages, a set of type-variables with fixed interpretations is assumed.
```
...
$C_i$ be a symbol,
$t_j: T_{i,j}$ for all $j\in\mathbb{N}_1^n$ and 
$i\in\mathbb{N}_1^n.$
\end{letIn}
...
```{block2, type="myexample",latex.options="_"}
\begin{letIn}$T ::= C \ a$ be a type application.\end{letIn}
