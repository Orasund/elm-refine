...
The semantics for `<type>` are equivalent to the curresponding inference rules. While both directions are valid, we will only need to show that the judgment $\Gamma\vdash t:T$ ensures $\semantic{t}_\Gamma = T$.

```{theorem}
\begin{letIn}
$\Gamma$ be type context, $t \in\mf{<type>}$ and $T\in\mathcal{T}$. 
\end{letIn}
The Judgement $\Gamma\vdash t:T$ can be derived if \(\semantic{$t$}_\Gamma = T\).
```
```{proof}
...
\item\textbf{case} $t = \mf{"\{"} \ \mathit{ltf} \ \mf{"\}"}$, for $\mathit{ltf}\in\mf{<list-type-field>}$: By induction over $\mf{<list-type-field>}$, assume $ltf = \mf{""}$, then $\Gamma\vdash ltf:\{ \ \}$ and \(\semantic{$\mathit{lft}$}_\Gamma=\{ \ \}\). Next, assume for given $\mathit{ltf}_1\in\mf{<list-type-field>}$ that $\Gamma\vdash \mathit{ltf}_1:\{a_1:T_1,\dots,a_n:T_n\}$ and \(\semantic{$\mathit{ltf}_1$}_\Gamma=\{a_1:T_1,\dots,a_n:T_n\}\) for given $T_i\in\mathcal{T},a_i\in\mathcal{V}$ and $i\in\mathbb{N}_1^n, n\in\mathbb{N}_0$.  Let $lft = a_0 \ \mf{":"} \ T_0 \ \mf{","} \ lft_1$ for given $a_0\in\mathcal{V}$ and $T_0\in\mathcal{T}$. Then $\Gamma\vdash \mathit{lft} : \{a_0 : T_0,a_1 : T_1\dots a_n:T_n \}$ and \(\semantic{$\mathit{lft}$}_\Gamma = \{a_0 : T_0,a_1 : T_1\dots a_n:T_n \}\). We can therefore conclude that for $\Gamma\vdash \mathit{ltf}:T$ for inductively derived $T\in\mathcal{T}$ that $\Gamma\vdash t : T$ and \(\semantic{$t$}_\Gamma= T\).
...
\item\textbf{case} $t = c \ l$ for $l\in\mf{<list-type>}$ and $c\in\mf{<upper-var>}$ with $(c,T')\in\Gamma$ with $T'\in\mathcal{T}$: By induction over $\mf{<list-type>}$, assume $l = \mf{""}$, then $\Gamma\vdash l:()$ and \(\semantic{$l$}_\Gamma = ()\). Next, assume for given $l_1\in\mf{<list-type>}$ that $\Gamma\vdash l_1:(T_1,\dots,T_n)$ for $T_i\in\mathcal{T}$,$i\in\mathbb{N}_1^n$ and $n\in\mathbb{N}_0$. Let $l = \ t_0 \ l_1$ for $t_0\in\mf{<type>}$ such that $\Gamma\vdash t_0 : T_0$ and \(\semantic{$t_0$}_\Gamma=T_0\) and $T_0\in\mathcal{T}$, then $\Gamma\vdash l : (T_0,T_1,\dots,T_n)$. We can therefore assume $\Gamma\vdash l:(T_0,\dots,T_n)$ and \(\semantic{$l$}_\Gamma=(T_1,\dots,T_n)\) for $T_i\in\mathcal{T}$,$i\in\mathbb{N}^n$ and $n\in\mathbb{N}_0$. Then $\Gamma\vdash t:\overline{T'} \ T_1 \ \dots \ T_n$ and \(\semantic{$t$}_\Gamma =\overline{T'} \ T_1 \ \dots \ T_n\), where $\overline{T'}$ represents the application constructor of $T'$
