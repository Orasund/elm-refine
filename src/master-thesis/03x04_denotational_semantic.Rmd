...
\item\textbf{Case} $t = \mf{"\{"} \ \mathit{ltf} \ \mf{"\}"}$, for $\mathit{ltf}\in\mf{<list-type-field>}$: We start with induction over $\mf{<list-type-field>}$.
  \begin{itemize}
  ...
  \end{itemize}
We can therefore conclude that for $\Gamma\vdash \mathit{ltf}:T$ for inductively derived $T\in\mathcal{T}$ that $\Gamma\vdash t : T$ and \(\semantic{$t$}_\Gamma= T\).
...
\item\textbf{Case} $t = c \ l$ for $l\in\mf{<list-type>}$ and $c\in\mf{<upper-var>}$ with $(c,T')\in\Gamma$ with $T'\in\mathcal{T}$: we start with induction over $\mf{<list-type>}$.
  \begin{itemize}
  ...
  \end{itemize}
We can therefore conclude that $\Gamma\vdash l:(T_0,\dots,T_n)$ and \(\semantic{$l$}_\Gamma=(T_1,\dots,T_n)\) for $T_i\in\mathcal{T}$,$i\in\mathbb{N}^n$ and $n\in\mathbb{N}_0$. Then $\Gamma\vdash t:\overline{T'} \ T_1 \ \dots \ T_n$ and \(\semantic{$t$}_\Gamma =\overline{T'} \ T_1 \ \dots \ T_n\), where $\overline{T'}$ represents the application constructor of $T'$
