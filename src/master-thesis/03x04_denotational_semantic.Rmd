...
\begin{letIn}
$\Gamma$ be a type context and let $\Delta\Theta,\Theta_1,\Theta_2,\Theta_3$ be variable contexts. Let $p,p_1,p_2\in\mf{<pattern>}$, $\mathit{lpl}\in\mf{<list-pattern-list>}$, $\mathit{lps}\in\mf{<list-pattern-sort>}$ and $\mathit{lpv}\in\mf{<list-pattern-vars>}$. Let $b\in\mf{<bool>}$ and $i\in\mf{<int>}$. Let $c,a\in\mathcal{V}$.
\end{letIn}
...
$$
\begin{aligned}
\text{match}_{\Gamma,\Delta,\Theta}:&(\bigcup_{T\in\mathcal{T}} \text{value}_\Gamma(\mathit{List} \ T))\times\mf{<list-pattern-list>}\\
\text{match}_{\Gamma,\Delta,\Theta}(s,\mf{""}):\Leftrightarrow& [ \ ] = s\\
\text{match}_{\Gamma,\Delta,\Theta_3}(s,p \ \mf{","} \ \mathit{lpl}):\Leftrightarrow& [a_0,\dots,a_n] = s\\
    &\land \text{match}_{\Gamma,\Delta,\Theta_1}(a_0,p) \land \text{match}_{\Gamma,\Delta,\Theta_2}(a_1,\dots,a_n,\mathit{lpl})\\
