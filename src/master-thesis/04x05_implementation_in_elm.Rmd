...
```
 ...
 We obtain simple conditions from the `split` function. This is a 1:1 implementation of the split function previously described. We will now go through its definition.
 ...
 * `weaken` says if we are currently weakening a condition. If this is set to `Nothing` then we are in the `solve` function, else its `Just` an `Int`, namely the index of the predicate that we are currently investigating.
...
If the incoming result is `False` then the SMT statement is unsatifiable. Thus, we remove the predicate. If no other predicate exists, we finish the `weaken` function by setting `model.weaken` to `Nothing`.
