...
\begin{letIn} $\Delta$ be a type context, $T\in\mathcal{T}$, $a_i\in\mathcal{V}$ for $i\in\mathbb{N}_0^n$. Let $T'$ be the mono type of $T$.
\end{letIn}

We say $\forall a_1 \dots \forall a_n.T'$ is a \textit{uniquely quantified} poly type of $T$ in $\Delta$, iff the following holds:
\[(\_,\forall a_1 \dots \forall a_n.T')\in\Delta_2 \land \{a_1,\dots,a_n\}=\text{free}(T')\land \forall i\in\mathbb{N}_0^n.(a_i,\_)\not\in\Delta_2\]
...
&\Delta_1\cup\small\begin{Bmatrix*}[l]
    ...
    \land \ T' \text{ is a uniquely quantified poly type of } T \text{ in } \Delta_2
