...
\item\textbf{Case} $e = e_1 \ \mf{"|>"} \ e_2$ for given $e_1,e_2\in\mf{<exp>}$: By the premise of the inference rule we assume $\Gamma,\Delta\vdash e_1: T_1$ and $\Gamma,\Delta\vdash e_2: T_1 \to T$ for $T_1\in\mathcal{T}$ can be derived and by induction hypothesis of Theorem \ref{thm:soundness_exp} \(\semantic{$e_1$}_{\Gamma,\Delta'} \in \text{value}(\overline{\Gamma}(T_1))\) and \(\semantic{$e_2$}_{\Gamma,\Delta'} \in \text{value}(\overline{\Gamma}(T_1\to T))\). Then \(\semantic{$e_1$ \ "|>" \ $e_2$}_{\Gamma,\Delta'} = \semantic{$e_2$}_{\Gamma,\Delta'}(\semantic{$e_1$}_{\Gamma,\Delta'})\) and thus the conclusion holds.
\item\textbf{Case} $e = e_1 \ \mf{">>"} \ e_2$ for given $e_1,e_2\in\mf{<exp>}$: Then $T=T_1\to T_3$ for $T_1,T_3\in\mathcal{T}$. By the premise of the inference rule we assume $\Gamma,\Delta\vdash e_1: T_1 \to T_2$ for $T_2\in\mathcal{T}$ and $\Gamma,\Delta\vdash e_2: T_2 \to T_3$ can be derived. Then by induction hypothesis of Theorem \ref{thm:soundness_exp} we know \(\semantic{$e_1$}_{\Gamma,\Delta'} \in \text{value}(\overline{\Gamma}(T_1 \to T_2))\) and \(\semantic{$e_2$}_{\Gamma,\Delta'} \in \text{value}(\overline{\Gamma}(T_2 \to T_3))\). Thus, by the definition of the semantics the conclusion holds analogously to the cases above.
